...
@router.get("/check-isbn/{isbn}")
async def check_isbn(isbn: str, repo: BookRepository = Depends(get_book_repo)):
    """Check if an ISBN already exists in the library."""
    book = await repo.get_by_isbn(isbn)
    if book:
        return {"exists": True, "title": book.title, "book_id": book.id}
    return {"exists": False}
...
async def add_book(data: BookCreate, repo: BookRepository = Depends(get_book_repo)):
    ...
    if data.source == "hardcover" and data.hardcover_id:
        ...
        try:
            book = await fetch_book_details(
                data.open_library_key, search_data=search_data
            )
        except OpenLibraryError as e:
        ...
        if not data.title:
            raise HTTPException(
                status_code=400, detail="Title is required for manual entry"
            )
        ...
        raise HTTPException(
            status_code=400, detail="Invalid source or missing required fields"
        )
...
async def list_books(
    limit: int = 50, offset: int = 0, repo: BookRepository = Depends(get_book_repo)
):
...
async def get_books_by_grade(grade: str, repo: BookRepository = Depends(get_book_repo)):
...
async def get_book(book_id: str, repo: BookRepository = Depends(get_book_repo)):
...
async def update_book(
    book_id: str, data: BookUpdate, repo: BookRepository = Depends(get_book_repo)
):
...
async def delete_book(book_id: str, repo: BookRepository = Depends(get_book_repo)):
...
async def delete_cover(book_id: str, repo: BookRepository = Depends(get_book_repo)):
